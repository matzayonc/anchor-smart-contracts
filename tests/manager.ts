...
export async function generateToken(value: number, usersKey: PublicKey) {
  const associatedToken = await program.account.token.associatedAddress(authority, usersKey)
...
export async function getTokenAmount(user: PublicKey): Promise<number> {
  const { amount } = (await program.account.token.associated(authority, user)) as { amount: number }

  return amount
}

export async function recalculateToken(user: PublicKey): Promise<void> {
  const associatedToken = await program.account.token.associatedAddress(authority, user)
...
export async function getTokenWithdrawable(user: PublicKey): Promise<number> {
  ...
  const { withdrawable } = (await program.account.token.associated(authority, user)) as {
    withdrawable: number
  }
